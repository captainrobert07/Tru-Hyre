...
| 14 | **Security / data-egress hardening track** (dev/test) — **DONE (every egress vector checked, verified live + locked)** | The run's headline (alongside the XSS find). Candidate data is now verified non-leaking across ALL paths: stored-XSS via `href` (R9, closed iter 51); PII in prod logs (R12, closed iter 118); raw `error.message` genericized in the React error UI (iter 108) AND the PII file API (iter 138); the `/api/files` per-role IDOR boundary locked (iter 137, E2E); the full server-action auth-guard census (iter 128, 40/40 authenticated + owner-scoped). This is the **"technical controls already in place"** evidence for the R5 data-residency/audit posture doc — the back-door complement to R3's SSO front-door. | STRATEGY iter 139 synthesis; R9/R12, DEV iter 128, test iter 137 |
